	...
	var trigger=0, name, age, sex, education, mathematic, asu, cutting, programming; // обьявляем переменные которые понадобятся в ходе работы
		...
		function myFunction() { // эта функция автоматически проверяет подходит ли наш кандидат: "кандидаты со знанем программирования и высшим
					//образованием + cutting".
	    	var x = document.getElementById("div1");
		if (trigger==1) {
		if (document.getElementById("education").checked ==true && 
		document.getElementById("programming").checked ==true && 
		document.getElementById("cutting").checked ==true)
		
		{
       		var text ='<p class="stroke">ВЫ НАМ ПОДХОДИТЕ!</p>'
		x.innerHTML = text;
		x.style.display = "block";
		}
		else 	{
        	var text ='<p class="stroke">ВЫ НАМ НЕ ПОДХОДИТЕ!</p>'
		x.innerHTML = text;
		x.style.display = "block";
			}
		}
